...
export const performIncrementalSync = internalAction({
  handler: async (ctx): Promise<SyncResult | void> => {
    try {
      ...
      const syncState = await ctx.runQuery(internal.todoist.internalQueries.getSyncState.getSyncState);
      if (!syncState?.last_sync_token) {
        return ctx.runAction(internal.todoist.sync.runInitialSync.runInitialSync);
      }
      ...
      // Update sync token
      await ctx.runMutation(internal.todoist.internalMutations.updateSyncToken.updateSyncToken, {
