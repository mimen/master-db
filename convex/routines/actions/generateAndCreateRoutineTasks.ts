 ...
 */
export const generateAndCreateRoutineTasks = action({
  args: {
  ...
  > => {
    try {
      ...
      const taskSpecs = await ctx.runMutation(
        internal.routines.mutations.generateTasksForRoutine,
        { routineId }
      );
      ...
      for (const spec of taskSpecs) {
        const result = await ctx.runAction(
          internal.routines.publicActions.createRoutineTaskInTodoist,
          {
