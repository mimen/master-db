...
import { flagValue, hasFlag, REPO_ROOT, run, shellQuote, tryRun } from "./deployment/runtime";
...
let hadCandidateErrors = false;
const manifests = explicitDryRun ? [] : await readRemoteManifests();
if (manifests.length > 0) await run(["git", "-C", REPO_ROOT, "fetch", "-q", "origin"]);
...
for (const manifest of manifests) {
  try {
    ...
    hadCandidateErrors = true;
    console.error(`ERROR ${manifest.branch}: ${error instanceof Error ? error.message : String(error)}`);
  }
}
if (hadCandidateErrors) process.exitCode = 1;

async function processManifest(discoveredManifest: BranchManifest): Promise<void> {
  ...
  try {
    ...
    const portReleaseScript = previewPortReleaseScript();
    const remoteScript = [
      "set -euo pipefail",
      ...
      `python3 -c ${shellQuote(portReleaseScript)} ${shellQuote(`${remoteRoot}/.port-locks/${manifest.previewPort}`)} ${shellQuote(manifest.branchHash)} 1`,
...
async function readRemoteManifests(): Promise<readonly BranchManifest[]> {
  ...
  const manifests: BranchManifest[] = [];
  for (const value of serialized) {
    try {
      manifests.push(parseBranchManifest(value));
    } catch (error) {
      hadCandidateErrors = true;
      console.error(`ERROR invalid branch manifest: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  return manifests;
}

async function isMerged(sha: string): Promise<boolean | null> {
  const resolved = await tryRun(["git", "-C", REPO_ROOT, "cat-file", "-e", `${sha}^{commit}`]);
