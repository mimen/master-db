...
function seeded(): string {
  ...
  db.exec(
    `INSERT INTO triage_clear_event (chat_guid, message_guid, reason, cleared_at) VALUES ('b', 'msg-1', 'dismiss', 1)`,
  );
  db.close();
  return path;
}
...
describe("one-triage-gesture verification", () => {
  test("passes on a correctly migrated database", () => {
    const checks = verify({ db: migrated(), expectRows: 3, expectClearEventsAtLeast: 1 });
  ...
  test("fails when a retired column is still present", () => {
    ...
    const checks = verify({ db: path, expectRows: 3 });
  ...
  test("fails when the clear-event log was truncated", () => {
    ...
    const checks = verify({ db: path, expectClearEventsAtLeast: 1 });
